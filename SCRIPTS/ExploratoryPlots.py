...
plt.title('Year vs Sentiment Score', fontsize=16)
plt.xlabel('Year', fontsize=12)
...
# 1. Movies that pass (rating == 3) vs those that fail (rating < 3)
...
# 2. Movies that pass the Bechdel test over time (entire dataset)
...
# 3. Colorful pair plot with title
...
# Count of Bechdel test ratings (0, 1, 2, 3)
plt.figure(figsize=(10, 6))
sns.countplot(x='rating', data=df, palette='coolwarm')
plt.title('Distribution of Bechdel Test Ratings', fontsize=16)
plt.xlabel('Bechdel Test Rating', fontsize=12)
